...
|------|---------|---------|-----------|-----------|
| step 1-50 | 0.173 | 0.154 | 0.955 | 0.828 |
| step 51-100 | 0.160 | 0.158 | — | — |
| step 101-150 | **0.227** | 0.180 | — | — |
| step 151-200 | 0.190 | 0.160 | 0.966 | 0.828 |
| step 201-225 | 0.189 | 0.137 | — | — |

多轮训练的 reward 波动始终大于单轮（std 高 10-30%），且差距在训练中后期（step 100+）更明显。这是因为多轮交互的 token 消耗和纠错结果更不稳定——同一 batch 内可能混合"T1 全过"和"T1 全挂但 T2 纠错"两种极端情况，导致 reward 方差更大。
...
|-------|---------|---------|---------|---------|---------|-----------|
| 1-25 | 14.9% | **8.3%** | — | 11.4% | 21.9% | 0.093 |
| 26-50 | 13.2% | 16.4% | — | 11.5% | 27.4% | 0.135 |
| 51-75 | 17.2% | 33.7% | — | 8.0% | 45.1% | 0.327 |
| 76-100 | 17.7% | 36.9% | — | 7.4% | 48.1% | 0.359 |
| 101-124 | 14.6% | 41.4% | — | 8.0% | 49.9% | 0.365 |
| 201-225 | 15.1% | 40.2% | — | 7.2% | 49.2% | 0.367 |
| 226-250 | 13.9% | 38.5% | — | 6.2% | 47.0% | 0.355 |
| 251-275 | 17.4% | 42.4% | — | 5.3% | 52.4% | 0.416 |
| 276-300 | 16.7% | **44.1%** | — | 4.8% | 53.4% | 0.429 |

> 注：step 1-225 日志因训练重启有部分重叠数据，但 T2 纠错率的上升趋势是可靠的。step 200-300 为续训数据，无重叠。

**关键观察：T1 成功率在训练全程基本不变（10-19%），提升主要来自纠错能力。** 模型在训练中的首次代码生成能力改善缓慢（temperature>0 的采样下），但 T2 纠错率从 8% 稳步提升到 44%，说明模型确实在逐步学会"看到错误反馈后修正代码"。
...
- **r8192 下最佳**（mt_s150）：HumanEval+ 0.780（**+39.0%**）

#### 2. 多轮 GRPO 收敛更快

多轮 GRPO 的滑动平均 reward 始终高于单轮，step 100 后差距达 +0.08，step 200 后达 +0.32。多轮纠错提供了大量额外正 reward（T2 贡献 66%），帮助模型更快摆脱初期负 reward 区域。首次 reward>0.3 多轮仅需 step 36，单轮需要 step 79。

但更快收敛**并未转化为更好的最终评估性能**（最佳 HEval+ 持平 0.732），可能因为多轮训练的 KL 散度增长更快（step 300 时 0.055 vs 单轮 0.031），后期过拟合风险更高（mt_s300 HEval+ 下降到 0.677）。
...
- **不是 reward 设计问题**：训练中 T2 NO_CODE 仅 0.2%，97.7% NO_CODE 在首轮，不存在"首轮有代码但后续因 budget 截断被扣 -1"的大规模误判
- **更可能是优化目标复杂度问题**：多轮训练需要同时优化"首次生成"和"纠错"两个能力，在有限步数内两方面都没有达到最优；单轮训练专注提升首次生成质量，间接也提升了代码理解能力，使其在纠错时也表现更好
...
所有 checkpoint 总共只有 10 道题曾被纠错成功，且都是"大部分 checkpoint T1 能做对"的中等题。纠错本质上是对 T1 偶尔失误的保险，而不是解决难题的能力。不同 checkpoint 纠错的题目几乎没有重叠，说明纠错成功率高度依赖生成随机性。
...
1. **GRPO 有效**：从 Qwen3-1.7B 基线 56.1% 提升到 73.2%（HumanEval+），增幅 30.5%；r8192 下最佳 78.0%（+39.0%）
...
4. **单轮训练模型的纠错率更高**：st_s300 T2 纠错率 35% vs mt_s150 的 21%，单轮训练专注于生成质量，间接提升了纠错能力
5. **多轮 reward 设计本身不是问题**：训练中 T2 NO_CODE 仅 0.2%，97.7% NO_CODE 在首轮，不存在大规模"后续轮次因 budget 截断被误判"的情况
6. **评估纠错率的瓶颈是 greedy decoding + thinking 过长**：训练中 T2 纠错率 42.8%，评估 9-35%，r8192 下改善有限（纠错数仅 +2-3），说明需要从根本上提升模型的纠错能力而非仅增大 budget
