...
> std 和 range 均基于 wandb `critic/rewards/mean`（每步一个 batch 均值），在 50 步窗口内计算。std 衡量步间波动的离散程度，range = max - min 衡量极端值的差距。
...
多轮训练的 reward 波动始终大于单轮（std 高 10-30%），且差距在训练中后期（step 100+）更明显。step 101-150 区间多轮 std 达到峰值 0.227（单轮仅 0.180），对应纠错率快速爬升的阶段——模型尝试纠错但成功率不稳定，导致 reward 方差最大。range 方面多轮始终 ≥0.75，单轮全程在 0.64-0.83 区间内波动。
...
两种训练方式的截断率都呈下降趋势（模型逐渐学会更短的输出），波动幅度相近。
...
- **多目标优化的可能代价**：多轮训练相比单轮训练，除了需要提升代码生成能力，还需要额外掌握纠错能力，但 1.7B 参数量的模型容量可能不足以同时学好这两个能力。从结果看，多轮训练的 T1 成功率相比单轮并没有明显优势（11-18%，波动较大），纠错能力虽然在训练中逐步提升（T2 纠错率 2%→45%），但评估时未能转化为更好的最终性能。同时，多轮训练的 KL 散度增长更快（step 300 时 0.055，是单轮的 1.8x），这可能导致了更早的过拟合（mt_s300 单轮 HEval+ 从 0.732 降到 0.677）。相比之下，单轮训练只关注生成质量，优化目标更单一，KL 增长更慢，在评估时泛化表现反而更好
