...
- **多目标优化导致两头不靠**：多轮训练需要同时学"首次生成"和"纠错"，但 T1 成功率全程没有明显提升（10-18%），说明生成能力没有被充分优化；而 KL 散度却增长过快（step 300 时 0.055，是单轮的 1.8x），导致后期泛化变差（mt_s300 单轮 HEval+ 从 0.732 降到 0.677）。单轮训练只优化生成质量，KL 增长更慢，在评估时反而泛化更好
